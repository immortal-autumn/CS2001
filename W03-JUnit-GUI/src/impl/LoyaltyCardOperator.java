...
public class LoyaltyCardOperator extends AbstractFactoryClient implements ILoyaltyCardOperator {
    ...
    public void saveData() throws FileNotFoundException {
        String path = "user_profiles.information";
        File output = new File(path);
        try {
            PrintWriter writer = new PrintWriter(output);
            for (ILoyaltyCard card : registeredCard.values()) {
                writer.println(card.getOwner().getEmail());
                writer.println(card.getOwner().getName());
                writer.println(card.getNumberOfPoints());
                writer.println(card.getNumberOfUses());
